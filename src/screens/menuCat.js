...
const MenuCatScreen = ({navigation}) => {
  ...
  useEffect(() => {
    if(restaurantData){
      const menuCat = Object.keys(restaurantData?.["menu-categories"]).map(cat => {
        return {
  ...
  const handleTextToSpeech = (menuCat) => {
    console.log(menuCat[active], "SDfsvxcvxc")
  ...
  const handleSingleTap = () => {
    ...
    <Box
      ...
      {
        Boolean(menuCat.length) && menuCat.map((cat, idx) => {
          return <HStack key={cat?.id} borderRadius="md" mb="2" backgroundColor={colors.backgroundLight} justifyContent="space-between" borderWidth={3} borderColor = {active === idx ? 'red.400' : '#3b3b44'} alignItems={'center'} paddingX={3} paddingY={3}>
            <Text color={"white"}>{cat?.name}</Text>
            <Ionicons name="ios-arrow-down-circle-sharp" color={colors.headeline} size={30} />
          </HStack>
        })
      }
      <HStack></HStack>
